...
from .abi import RAI_ABI, RAI_PRICE_SNAP


class RaiPool:
    def __init__(self, pool, token, w3=None, abi=RAI_ABI):
        if not w3:
    ...
    def get_rate(self, i, underlying=False, block=None):
        ...
        if i == 0:
            rate = self.price_snap.snappedRedemptionPrice().call(**kw) // 10**9
        if i == 1:
    ...
    def fetch_stats(self, block='latest'):
        ...
        return {
            ...
            'virtual_price': is_deposited and self.pool.get_virtual_price_2().call(**kw),
            'timestamp': timestamp,
            'balances': balances,
            'rates': rates,
