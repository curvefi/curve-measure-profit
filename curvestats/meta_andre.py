...
from web3.exceptions import BadFunctionCallOutput, ABIEventFunctionNotFound
from . import TOKEN_ABI
from .abi import NEW_ABI

ABI = NEW_ABI.copy()
ABI += [{"name":"base_pool","outputs":[{"type":"address","name":""}],"inputs":[],"stateMutability":"view","type":"function","gas":2291}]  # noqa


class MetaPool:
    def __init__(self, pool, token, base_pool, w3=None, abi=ABI):
        if not w3:
        ...
        self.base_pool = self.w3.eth.contract(abi=ABI, address=base_pool).functions
